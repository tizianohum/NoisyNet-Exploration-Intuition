...
def main(cfg: DictConfig) -> float:
    ...
    for useNoisyNet in [True]:
        if useNoisyNet:
            for useNoisyReduction in [True, False]:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                print(f"Training with NoisyNet: {useNoisyNet}, Noise Reduction: {useNoisyReduction}")
                for seed in range(0,30):
                    if not useNoisyReduction and seed < 15:
                        continue
                    # 1) build env
                    ...
                    agent_kwargs = dict(
                        ...
                        seed=seed,
                        useNoisyNet=useNoisyNet,
                        useNoiseReduction=useNoisyReduction,
            ...
            useNoisyReduction = False  
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            print(f"Training with NoisyNet: {useNoisyNet}")
